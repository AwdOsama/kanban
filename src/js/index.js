...
(function () {
    ...
    <h3
      class="mb-2 font-semibold leading-snug ${isCompleted ? "text-slate-500 line-through" : "text-slate-800"}"
    >
      ${task.title}
    </h3>
    ...
    >
      ...
      ${task.dueDate
            ? `
            <div
              class="flex items-center gap-1.5 ${isOverdue && !isCompleted ? "text-red-500" : ""}"
            >
  ...
  `;
    ...
    // EMPTY STATE
    function showEmptyState(taskList, container) {
  ...
  `;
    ...
    function updateCounters() {
        ...
        if (progressCount) {
            progressCount.textContent = `${progress} ${progress === 1 ? "task" : "tasks"}`;
        }
        if (completedCount) {
